  ...
  async validate(val, msg, arg) {
    ...
    if (matches) {
      try {
        const member = await msg.guild.members.fetch(
          await msg.client.users.fetch(matches[1])
        );
    ...
    return members.size <= 15
      ? `${disambiguation(
          members.map(
            (mem) =>
              `${escapeMarkdown(mem.user.username)}#${mem.user.discriminator}`
          ),
          "membres",
          null
        )}\n`
      : "Plusieurs membres ont été trouvés. S'il vous plait soyez plus spécifique.";
  }
