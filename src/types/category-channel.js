...
class CategoryChannelArgumentType extends ArgumentType {
  ...
  validate(val, msg, arg) {
    ...
    return channels.size <= 15
      ...
      : "Plusieures catégories ont été trouvées. S'il vous plait soyez plus spécifique.";
  }
...
function channelFilterExact(search) {
  return (chan) =>
    chan.type === "category" && chan.name.toLowerCase() === search;
}

function channelFilterInexact(search) {
  return (chan) =>
    chan.type === "category" && chan.name.toLowerCase().includes(search);
}
