  ...
  class CommandoMessage extends Message {
    ...
    async run() {
      ...
      if (!hasPermission || typeof hasPermission === "string") {
        const data = {
          response:
            typeof hasPermission === "string" ? hasPermission : undefined,
        };
      ...
      if (!args && this.command.argsCollector) {
        ...
        if (collResult.cancelled) {
          ...
          return this.reply("La commande a été annulée.");
        }
        ...
        if (response instanceof Array) {
          for (let i = 0; i < content.length; i++) {
            if (response.length > i)
              promises.push(
                response[i].edit(`${prepend}${content[i]}`, options)
              );
            else
              promises.push(
                response[0].channel.send(`${prepend}${content[i]}`)
              );
          }
        } else {
    ...
    deleteRemainingResponses() {
      for (const id of Object.keys(this.responses)) {
        ...
        for (
          let i = this.responsePositions[id] + 1;
          i < responses.length;
          i++
        ) {
