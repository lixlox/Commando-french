  ...
  async obtain(msg, val, promptLimit = Infinity) {
      ...
      prompts.push(
        await msg.reply(stripIndents`
          ...
          empty
            ...
            : valid
            ? valid
            : `Vous avez fournis un/une ${this.label} invalide. Veuillez réessayer.`
        }
				${oneLine`
					Répondez avec \`cancel\` pour annuler la commande.
					${
            wait
              ? `Cette commande va être automatiquement annulée au bout de ${this.wait} secondes.`
              : ""
          }
				`}
			`)
    ...
    return {
      value: await this.parse(
        val,
        msg,
        answers.length ? answers[answers.length - 1] : msg
      ),
      cancelled: null,
      prompts,
      answers,
          ...
          prompts.push(
            await msg.reply(stripIndents`
              ...
              valid
                ? valid
                : oneLine`
							Vous avez fournis un/une ${this.label} invalide,
							"${escaped.length < 1850 ? escaped : "[Trop long à afficher]"}".
							Veuillez réessayer.
						`
            }
						${oneLine`
							Répondez avec \`cancel\` pour annuler la commande, ou \`finish\` pour finir à ce stade.
							${
                wait
                  ? `Cette commande va être automatiquement annulée dans ${this.wait} secondes.`
                  : ""
              }
						`}
					`)
          ...
          prompts.push(
            await msg.reply(stripIndents`
						...
						${oneLine`
							...
							${
                wait
                  ? `La commande va être automatiquement annulée dans ${this.wait} secondes, sauf si vous répondez.`
                  : ""
              }
						`}
					`)
