...
class Command {
  ...
  hasPermission(message, ownerOverride = true) {
    ...
    if (
      ...
      return `La commande \`${this.name}\` peut être seulement utilisée par les propriétaires du bot.`;
    }
    ...
    if (message.channel.type === "text" && this.userPermissions) {
      ...
      if (missing.length > 0) {
        if (missing.length === 1) {
          return `La commande \`${
            this.name
          }\` a besoin que vous ayez la permission "${
            permissions[missing[0]]
          }".`;
        }
        return oneLine`
					La commande \`${this.name}\` a besoin que vous ayez les permissions suivantes:
    ...
    switch (reason) {
      case "guildOnly":
        return message.reply(
          `La commande \`${this.name}\` doit être utilisée dans le salon d'un serveur.`
        );
      case "nsfw":
        return message.reply(
          `La commande \`${this.name}\` peut seulement être utilisée dans les salons NSFW.`
        );
      case "permission": {
        if (data.response) return message.reply(data.response);
        return message.reply(
          `Vous n'avez pas la permission d'utiliser la commande \`${this.name}\`.`
        );
      }
      case "clientPermissions": {
        if (data.missing.length === 1) {
          return message.reply(
            ...
            }" pour que la commande \`${this.name}\` fonctionne.`
          );
        }
        return message.reply(oneLine`
					J'ai besoin des permission suivantes pour que la commande \`${
            this.name
          }\` fonctionne:
      ...
      case "throttling": {
        return message.reply(
          `Vous ne pouvez pas utiliser la commande \`${
            this.name
          }\` pendant encore ${data.remaining.toFixed(1)} secondes.`
        );
      }
      default:
        return null;
  ...
  onError(err, message, args, fromPattern, result) {
    ...
    return message.reply(stripIndents`
			Une erreur s'est produit en executant la commande: \`${err.name}: ${
      err.message
    }\`
			Vous ne devez jamais recevoir une erreur comme ça.
			S'il vous plait contactez ${ownerList || "le propriétaire du bot"}${
      invite ? ` dans ce serveur: ${invite}` : "."
    }
		`);
  }
